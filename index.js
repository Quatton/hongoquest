...
async function handleEvent(event) {
  ...
  switch (event.type) {
    ...
    case "follow":
      ...
      console.log(Date.now() - new Date(2022, 4, 13, 5, 55).getTime());
      if (Date.now() - new Date(2022, 4, 13, 5, 55).getTime() > 0) {
...
//broadcastMessage
const eta_ms = new Date(2022, 4, 13, 5, 55).getTime() - Date.now();
console.log(eta_ms);
if (eta_ms > 0)
  setTimeout(() => {
    const message = [
