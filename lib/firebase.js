...
export async function writeLeaderBoard(mode, userId, time, wrong, name) {
...
function toTimeString(time) {
  ...
  const seconds = Math.floor(time / 1000) - 60 * minutes - 3600 * hours;
...
export async function endGame(userId) {
  ...
  switch (gameData.mode) {
    case 0:
      ...
      writeLeaderBoard(0, userId, time, gameData.wrong, userData.name);
      break;
    case 1:
      ...
      writeLeaderBoard(1, userId, time, gameData.wrong, userData.name);
      break;
    case 2:
      ...
      writeLeaderBoard(2, userId, time, gameData.wrong, userData.name);
      break;
  }
...
export async function loadLeaderboard(mode) {
  const leaderboard = await (
    await get(ref(database, `leaderboard/${mode}`))
  ).val();
...
export async function loadUserLeaderboard(userId) {
  ...
  if (userRanking.hard >= 10) {
    userLeaderboard.hard = Object.entries(leaderboard.hard).slice(0, 9);
    userLeaderboard.online.push([
      userId,
      Object.entries(leaderboard.hard)[userRanking.hard][1],
    ]);
  } else userLeaderboard.hard = Object.entries(leaderboard.hard).slice(0, 10);

  if (userRanking.easy >= 10) {
    userLeaderboard.easy = Object.entries(leaderboard.easy).slice(0, 9);
    userLeaderboard.online.push([
      userId,
      Object.entries(leaderboard.easy)[userRanking.easy][1],
    ]);
  } else userLeaderboard.easy = Object.entries(leaderboard.easy).slice(0, 10);

  if (userRanking.online >= 10) {
    userLeaderboard.online = Object.entries(leaderboard.online).slice(0, 9);
    userLeaderboard.online.push([
      userId,
      Object.entries(leaderboard.online)[userRanking.online][1],
    ]);
  } else
    userLeaderboard.online = Object.entries(leaderboard.online).slice(0, 10);
...
// loadUserLeaderboard("U20773423120786428dda6ca87797852b").then(console.log);
